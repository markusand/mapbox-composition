...
var index = map => {
	...
	const addNavigation = (config, position) => {
		addControl('navigation', new mapboxGl.NavigationControl(config), position);
	};

	const addScale = (config, position) => {
		addControl('scale', new mapboxGl.ScaleControl(config), position);
	};

	const addGeolocate = (config, position) => {
		addControl('geolocate', new mapboxGl.GeolocateControl(config), position);
	};

	const addAttribution = (config, position) => {
		addControl('attribution', new mapboxGl.AttributionControl(config), position);
	};

	const addFullscreen = (config, position) => {
		addControl('fullscreen', new mapboxGl.FullscreenControl(config), position);
	};
...
function useMarker(map, options = {}) {
	...
	const marker = new mapboxGl.Marker({ ...rest });
	let markerPopup;
...
function usePopup(...args) {
	...
	const popup = new mapboxGl.Popup(rest);
...
exports.useControls = index;
exports.useGeoJSON = useGeoJSON;

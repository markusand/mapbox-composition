...
var index = map => {
	...
	const addNavigation = ({ position = 'top-right', ...config } = {}) => {
		addControl('navigation', position, new mapboxGl.NavigationControl(config));
	};

	const addScale = ({ position = 'bottom-left', ...config } = {}) => {
		addControl('scale', position, new mapboxGl.ScaleControl(config));
	};

	const addGeolocate = ({ position = 'top-right', ...config } = {}) => {
		addControl('geolocate', position, new mapboxGl.GeolocateControl(config));
	};

	const addAttribution = ({ position = 'bottom-right', ...config } = {}) => {
		addControl('attribution', position, new mapboxGl.AttributionControl(config));
	};

	const addFullscreen = ({ position = 'top-right', ...config } = {}) => {
		addControl('fullscreen', position, new mapboxGl.FullscreenControl(config));
	};
